		...
		return new Promise(resolve=> {
			this.getStat(vote_id).then(stat=> {
				...
				let result_message = "<b>Результаты:</b>\n" +
					...
					`Средний между полем и штабом: ${(((stat.result.hq.count?(stat.result.hq.summary / stat.result.hq.count):0) + (stat.result.field.count?(stat.result.field.summary / stat.result.field.count):0)) / (~~!!stat.result.field.count+~~!!stat.result.hq.count)).toFixed(1)}\n` +
					"Гонорар авторам:\n" +
